	...
	switch ($_GET["op"]) {
      ...
      $cod_marca = $_POST["txtCodMarca"];
			...
			if(empty($_POST["txtIdUnidad_Medida"])){
				...
				if($objUnidad_Medida->Modificar($idunidad_medida, $nombre,$prefijo,$cod_marca)){
            ...
            $i = 1;
     		while ($reg = $query_Tipo->fetch_object()) {
     			$data[] = array(
                    ...
                    "2"=>$reg->nombre,
                    "3"=>$reg->prefijo,
                    "4"=>'<button class="btn btn-warning" data-toggle="tooltip" title="Editar" onclick="cargarDataUnidad_Medida('.$reg->idunidad_medida.',\''.$reg->nombre.'\',\''.$reg->prefijo.'\')"><i class="fa fa-pencil"></i> </button>&nbsp;'.
