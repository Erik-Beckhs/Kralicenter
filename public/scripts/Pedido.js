...
function init() {
    ...
    $('#txtIdSucursal').val(localStorage.getItem('idSucursal'));
...
// OBTENEMOS EL ID DE TICKET
  function GetPrimerIDTicket() {
    ...
    //   debugger;
       ...
       $.post("./ajax/PedidoAjax.php?op=GetPrimerIDTicket",data,  function(r){
  ...
  Cliente_Add_Recoger();
    ...
    function GenerarVenta(e){
      ...
      if($('#txtNum_Documento').val()==0){
        swal("KRALICENTER","Debe seleccionar un cliente", "warning");
        return;
      }

      var idVenta = $("#txtIsProforma").val();

      var tipoComprobante = $("#cboTipoComprobanteVta").val();
  ...
  if (elementos.length > 0) {
     ...
     swal("KRALICENTER","Debe ingresar al menos un artículo", "warning");
     return;
  }
}
else {
